...
class XIQSE_ControlPolicyRole(ControlPolicyRoleDetailsWebElements):
    ...
    def _right_click_roles_tree(self):
        ...
        if roles_node:
            self.utils.print_info("Right Clicking 'Roles' node in the Control->Policy->Roles tree")
            ret_val = self.auto_actions.right_click(roles_node)
        else:
            self.utils.print_info("Unable to find 'Roles' node in the Control->Policy->Roles tree")
    ...
    def _click_create_role_action(self):
        ...
        if create_role_action:
            self.utils.print_info("Selecting 'Create Role...' action in the Control->Policy->Roles tree menu")
            ret_val = self.auto_actions.click(create_role_action)
            sleep(2)
        else:
            self.utils.print_info("Unable to find 'Create Role' action in the Control->Policy->Roles tree menu")
            self.screen.save_screen_shot()
        return bool(ret_val > -1)

    def _verify_create_role_window(self):
        self.utils.print_info("Verifying 'Create Role' window is displayed")
        create_role_window = self.create_role_elements.get_window()
        if create_role_window:
            self.utils.print_info("Found 'Create Role' window")
            return True
        else:
            self.utils.print_info("Unable to find 'Create Role' window")
            self.screen.save_screen_shot()
            return False
    ...
    def _enter_create_role_name(self, name):
        ...
        if (name_input):
            ...
            self.utils.print_info("Unable to find 'Name' input field in Create Role Window")
            self.screen.save_screen_shot()
    ...
    def _click_create_role_ok(self):
        ...
        if (ok_button):
            self.utils.print_info("Clicking 'OK' button in Create Role Window")
            ret_val = self.auto_actions.click(ok_button)
        else:
            self.utils.print_info("Unable to find 'OK' button in Create Role Window")
            self.screen.save_screen_shot()
        return bool(ret_val > -1)
    ...
    def _expand_roles_tree(self):
        ret_val = -1
        self.utils.print_info("Checking if Control->Policy->Roles/Services->Roles tree is expanded")
        if self.role_tree_elements.is_roles_tree_expanded():
            ret_val = 1
        else:
            expander = self.role_tree_elements.get_roles_tree_expander_arrow()
            if expander:
                self.utils.print_info("Expanding Control->Policy->Roles/Services->Roles tree")
                ret_val = 1
            else:
                self.utils.print_info("Unable to find Control->Policy->Roles/Services->Roles tree expander arrow")
                self.screen.save_screen_shot()
        return ret_val
    ...
    def _click_delete_menu_action(self):
        ...
        if delete_role_action:
            self.utils.print_info("Selecting 'Delete' action in the Control->Policy tree node menu")
            ret_val = self.auto_actions.click(delete_role_action)
            sleep(2)
        else:
            self.utils.print_info("Unable to find 'Delete' action in Control->Policy tree node menu")
            self.screen.save_screen_shot()
        return bool(ret_val > -1)
    ...
    def _click_confirm_delete_yes(self):
        ...
        if confirm_yes_button:
            self.utils.print_info("Clicking 'Yes' button in the Control->Policy->Confirm Delete dialog")
