...
class Devices:
        ...
        try:
            ...
            if rows:
                    ...
                    if device_type and device_type in row.text:
                        ...
                        for cell in cells:
                            ...
                            if re.search(r'field-\w*', cell.get_attribute("class")):
                                try:
                                    label = \
                                    re.search(r'field-\w*', cell.get_attribute("class")).group().split("field-")[-1]
                                except Exception:
                                    label = 'OOPS'
                                device_detail_dict[label] = cell.text
                            else:
                ...
                self.utils.print_info(
                    f"List of Serial Numbers of Devices with device type {device_type}: {prev_dev_list}")
            else:
                self.utils.print_info("No rows present")
                ...
                if tooltip_text:
                    if "Your account does not have permission to perform that action" in tooltip_text:
                        self.auto_actions.click_reference(
                            self.devices_web_elements.get_actions_network_policy_close_button)
                        sleep(5)
                        return False
                return True
            else:
    ...
    def _update_network_policy(self, update_method="Delta", **kwargs):
        ...
        if update_method == "Delta":
            ...
            if update_tooltip_msg3 in tool_tp_text:
                ...
                self.auto_actions.click_reference(
                    self.devices_web_elements.get_action_assign_network_policy_dialog_cancel_button)
            ...
            if entry_type == "CSV":
                if csv_location:
                    ...
                    if upload_button:
                        ...
                        self.auto_actions.click_reference(
                            self.devices_web_elements.get_devices_add_devices_cancel_button)
        ...
        if device_type == "Stack":
            ...
            if search_result != -1:
                ...
                for serial in serials:
                    ...
                    if self.search_device_by_object(dict_serial, skip_refresh=True, skip_navigation=True,
                                                    ignore_failure=True) != -1:
                        device_found = 1
                        if self.wait_until_device_update_done(device_serial=serial, skip_navigation=True,
                                                              skip_refresh=True):
            ...
            while page_len:
                ...
                while retry > 0:
                    try:
                        ...
                        self.utils.print_info(
                            "Exception in row data (usually caused by XIQ device grid not finished), sleeping and retrying")
                        sleep(1)
                        retry = retry - 1;
                page_len = page_len - 1
    ...
    def search_device_by_object(self, *device_dict, select_device=False, skip_refresh=False, skip_navigation=False,
                                **kwargs):
        """
    ...
    def select_device(self, device_serial=None, device_name=None, device_mac=None,
        ...
        return self.search_device(device_serial=device_serial, device_mac=device_mac, device_name=device_name,
                                  select_device=True, skip_refresh=skip_refresh, skip_navigation=skip_navigation,
                                  **kwargs)
    ...
    def get_device_status(self, device_serial='default', device_name='default', device_mac='default', **kwargs):
        ...
        for key, value in device_keys.items():
            ...
            while attempt_count <= attempt_max:
                self.utils.print_info(
                    f"Trying to get device status from table cell. Attempt {attempt_count} of {attempt_max} attempts")
                try:
                ...
                self.utils.print_info(
                    f"Getting status from cell failed during attempt {attempt_count} of {attempt_max} attempts.  status = '{device_status}'")
                self.screen.save_screen_shot()
                attempt_count += 1
    ...
    def get_device_updated_status(self, device_serial=None, device_name=None, device_mac=None):
        ...
        self.utils.print_info('Getting device Updated Status using ',
                              *[x for x in {device_serial, device_name, device_mac} if x is not None])
    ...
    def delete_all_devices(self, **kwargs):
        ...
        if self.get_device_count() == 0:
            ...
            try:
                ...
                self.auto_actions.click_reference(
                    self.devices_web_elements.get_manage_devices_select_all_devices_checkbox)
                sleep(5)
    ...
    def wait_until_device_online(self, device_serial=None, device_mac=None, retry_duration=30, retry_count=20,
        ...
        if self.xapiDevices.is_xapi_enabled():
            return self.xapiDevices.xapi_wait_until_device_online(device_serial=device_serial, device_mac=device_mac,
    ...
    def wait_until_device_offline(self, device_serial=None, device_mac=None, retry_duration=30, retry_count=10,
        ...
        if self.xapiDevices.is_xapi_enabled():
            return self.xapiDevices.xapi_wait_until_device_offline(device_serial=device_serial, device_mac=device_mac,
                                                                   retry_duration=retry_duration,
                                                                   retry_count=retry_count, **kwargs)
    ...
    def _handle_credentials_global_popup(self):
        ...
        if the_dlg:
            ...
            if yes_btn:
                self.utils.print_info(
                    "Clicking 'Yes' in the account credential managed by global setting pop-up dialog")
                self.auto_actions.click_reference(
                    self.devices_web_elements.get_global_settings_management_dialog_yes_button)
            else:
                self.utils.print_info(
                    "Unable to find the Yes button in the account credential managed by global setting pop-up dialog")
                ret_val = -1
        else:
    ...
    def wait_until_device_removed_by_object(self, *device_dict, retry_duration=10, retry_count=30, **kwargs):
        ...
        while count <= retry_count:
            ...
            if self.search_device_by_object(device_keys, skip_refresh=True, skip_navigation=True,
                                            ignore_failure=True) == 1:
                self.utils.print_info(
                    f"Device with: {device_keys} is still present. Waiting for {retry_duration} seconds...")
                sleep(retry_duration)
                self.refresh_devices_page()
            else:
    ...
    def wait_until_device_managed(self, device_serial, retry_duration=30, retry_count=10, **kwargs):
        ...
        if self.xapiDevices.is_xapi_enabled():
            return self.xapiDevices.xapi_wait_until_device_managed(device_serial=device_serial,
                                                                   retry_duration=retry_duration,
    ...
    def get_device_management_ip_address(self, device_serial=None, device_name=None, device_mac=None, **kwargs):
        ...
        if self.xapiDevices.is_xapi_enabled():
            return self.xapiDevices.xapi_get_device_management_ip_address(device_serial=device_serial,
                                                                          device_mac=device_mac, **kwargs)
    ...
    def quick_onboarding_cloud_manual(self, device_sn, device_make, location, policy_name=None):
        ...
        if 'voss' in device_make.lower():
            ...
            if self.devices_web_elements.get_device_auto_detection_cloudIqEngineRadio():
                ...
                self.auto_actions.click_reference(
                    self.devices_web_elements.get_device_auto_detection_cloudIqEngineRadio)
            else:
    ...
    def update_device_auto_template(self, device_mac, name_stack_template, **kwargs):
        ...
        if self.auto_actions.click_reference(
                self.devices_web_elements.get_create_auto_template_update_device_click) == -1:
            ...
            try:
                for column_header in column_headers:
                    if column_name in column_header.text:
                        ...
                        sleep(5)
                        self.screen.save_screen_shot()
                        self.utils.print_info("Column Tooltip: ",
                                              self.devices_web_elements.get_ui_tool_tip_inner().text)
    ...
    def change_device_onboarding_date(self, ip_dest_ssh, user_dest_ssh, pass_dest_ssh, days, serial_number, owner_id,
        ...
        output_cmd3 = self.cli.send_pxssh(spawn,
                                          "select created_at,updated_at,agent_id,owner_id from hm_device where owner_id={};".format(
                                              owner_id))
        output_cmd4 = ''
    ...
    def check_long_sn_or_legacy_sn_mapping(self, device_serial, ip_dest_ssh, user_dest_ssh, pass_dest_ssh,
        ...
        if len(device_serial) > 11:
            ...
            if "ahqa_id_rsa" not in output_cmd_ls:
                ...
                return -1
            output_cmd = self.cli.send_pxssh(spawn,
                                             "ssh -i .ssh/ahqa_id_rsa ahqa@{}-console.qa.xcloudiq.com".format(rdc))
            self.utils.print_info(output_cmd)
            output_cmd1 = self.cli.send_pxssh(spawn, "sudo su -")
            output_cmd2 = self.cli.send_pxssh(spawn, "psqlsystemdb")
            output_cmd3 = self.cli.send_pxssh(spawn,
                                              "select * from extr_legacy_sn_mapping where serial_number='{}';".format(
                                                  device_serial))
            self.utils.print_info(output_cmd)
            self.utils.print_info(output_cmd1)
            self.utils.print_info(output_cmd2)
        ...
        if device_row:
            ...
            if re.search(r'\d+-\d+-\d+', device_updated_status):
                initial_timestamp = int(
                    datetime.timestamp(datetime.strptime(device_updated_status, "%Y-%m-%d %H:%M:%S")))
        ...
        try:
            if self.select_device(device_mac=device_mac):
                ...
                if checkbox_status == "true":  # If checkbox is selected we get string "true" otherwise we get None
                    self.utils.print_info(
                        "Upgrade IQ Engine and Extreme Network Switch Images checkbox is already checked")
                else:
                    self.utils.print_info("Selecting upgrade IQ Engine checkbox")
                    self.auto_actions.click_reference(self.device_update.get_upgrade_iq_engine_checkbox)
                ...
                if updateTo.lower() == "latest":
                    ...
                    if forceDownloadImage.lower() == "true":
                        if forceDownloadImage_checkbox_status is not None:  # If checkbox is selected we get string "true" otherwise we get None
                            self.utils.print_info(
                                "Perform upgrade if the versions are the same checkbox is already checked")
                        else:
                            self.utils.print_info("Selecting perform upgrade if the versions are the same checkbox")
                            self.auto_actions.click_reference(
                                self.device_update.get_upgrade_even_if_versions_same_checkbox)
                    else:
                        if forceDownloadImage_checkbox_status is not None:
                            self.utils.print_info(
                                "Perform upgrade if the versions are the same checkbox is checked - Unchecking")
                            self.auto_actions.click_reference(
                                self.device_update.get_upgrade_even_if_versions_same_checkbox)
                        else:
                            self.utils.print_info(
                ...
                # Case-2 : This flow is to perform firmware upgrade to a specific version if fails return -1
                    ...
                    if re.search(r'\d+.\d+.\d+.\d+', version):
                        ...
                        for opt in avilableImagesList:
                            if version in opt:
                                self.utils.print_info(
                                    "Version {} match the image {} from drop down".format(version, opt))
                                match_count += 1
                                updateToVersion = opt
                            else:
                                self.utils.print_info(
                ...
                if re.search(r'\d+-\d+-\d+', device_updated_status) or (device_updated_status == "") or (
                    ...
                    while True:
                        ...
                        if latest_timestamp > initial_timestamp:
                            self.utils.print_info("Device update is finished by just updating the timestamp to : ",
                                                  str(datetime.fromtimestamp(latest_timestamp)))
                            self.screen.save_screen_shot()
                            sleep(5)
                            break
                        elif count > 60:
    ...
    def wait_until_device_update_done(self, device_serial=None, device_mac=None, device_name=None, wait_time_in_min=15,
        ...
        while n_time <= int(wait_time_in_min * 4):
            ...
            if device_serial:
                ...
                self.utils.print_info(
                    "Updated status is: '" + str(update_status) + "' for the device_serial: " + str(device_serial))
            elif device_mac:
                update_status = self.get_device_details(device_mac, 'UPDATED')
                self.utils.print_info(
                    "Updated status is: '" + str(update_status) + "' for the device_mac: " + str(device_mac))
            elif device_name:
                update_status = self.get_device_details(device_name, 'UPDATED')
                self.utils.print_info(
                    "Updated status is: '" + str(update_status) + "' for the device_name: " + str(device_name))
    ...
    def change_manage_device_status(self, manage_type='MANAGE', device_serial=None, device_mac=None,
        ...
        if self.xapiDevices.is_xapi_enabled():
            return self.xapiDevices.xapi_change_manage_device_status(manage_type=manage_type,
                                                                     device_serial=device_serial, device_mac=device_mac,
    ...
    def get_cuid_and_viq_id(self, ip_dest_ssh, user_dest_ssh, pass_dest_ssh, owner_id, sw_connection_host):
        ...
        spawn = self.cli.open_pxssh_spawn(ip_dest_ssh, user_dest_ssh, pass_dest_ssh)
        output_cmd = self.cli.send_pxssh(spawn,
                                         "ssh -i .ssh/ahqa_id_rsa ahqa@{}-console.qa.xcloudiq.com".format(gdc[0]))
        self.utils.print_info(output_cmd)
        output_cmd1 = self.cli.send_pxssh(spawn, "sudo su -")
    ...
    def update_device_policy_config_reboot(self, device_serial):
        ...
        sleep(2)
        self.auto_actions.click_reference(
            self.devices_web_elements.get_switch_update_reboot_and_revert_warning_dialog_yes_button)
    ...
    def update_device_policy_config_image(self, device_serial):
        ...
        self.utils.print_info("Click on device update button")
        # self.auto_actions.click_reference(self.devices_web_elements.get_update_device_button)
        def _click_device_update_button():
            return self.auto_actions.click(self.devices_web_elements.get_update_device_button())
        self.utils.wait_till(_click_device_update_button, timeout=30, delay=20,
        ...
        def _click_image_option():
            return self.auto_actions.click(self.devices_web_elements.get_update_image_checkbox())
        self.utils.wait_till(_click_image_option, timeout=30, delay=20,
                             msg="Selecting image option")
                    ...
                    while cli_access_none:
                        ...
                        if action_dropdown:
                            ...
                            if cli_access:
                                ...
                                self.auto_actions.click_reference(
                                    self.device_actions.get_device_actions_advance_cli_access)
                                if self.device_actions.get_device_actions_cli_windows():
                                    ...
                                    self.auto_actions.send_keys(
                                        self.device_actions.get_device_actions_cli_windows_input(),
                                        "exec bypass-wan-hardening")
                                    self.utils.print_info("Click Apply button to send CLI to AP ...")
                                    self.auto_actions.click_reference(
                                        self.device_actions.get_device_actions_cli_windows_input_apply)
                                    self.utils.print_info("Close CLI windows ...")
                                    self.auto_actions.click_reference(
                                        self.device_actions.get_device_actions_cli_windows_close)
                                    return 1
                                else:
                                ...
                                self.utils.print_info(
                                    f"Cli access button is got: {cli_access}, refresh page and try again")
                                try_cnt1 += 1
                                self.utils.wait_till(self.cloud_driver.refresh_page(), is_logging_enabled=True)
                                if try_cnt1 == 10:
    ...
    def wait_for_policy_config_push_to_complete(self, device_serial, boot_wait_time=60, **kwargs):
        ...
        if reboot_res == 1:
            ...
            kwargs['fail_msg'] = 'Reboot for device with serial number: {} is NOT successful: {}'.format(device_serial,
                                                                                                         reboot_res)
            self.common_validation.failed(**kwargs)
            return -1
        ...
        if managed_res == 1:
            ...
            kwargs['fail_msg'] = 'Status for device with serial number: {} is NOT equal to managed: {}'.format(
                device_serial, managed_res)
            self.common_validation.failed(**kwargs)
            return -1
        ...
        if status_res == 'green':
            ...
            kwargs['fail_msg'] = 'Status for device with serial number: {} is NOT equal to green: {}'.format(
                device_serial, status_res)
            self.common_validation.failed(**kwargs)
            return -1
    ...
    def select_clone_device(self, device_serial, replacement_device_type, replacement_serial, option="disable",
        ...
        if clone_device:
            ...
            def _loading_clone():
                ...
                while loading_clone_configuration and "fn-hidden" not in loading_clone_configuration.get_attribute(
                        "class"):
            ...
            try:
                ...
                while warning_message_disconnected and "fn-hidden" in warning_message_disconnected.get_attribute(
                        "class"):
                    print(
                        "Still in loading clone configuration window..Waiting for Perform Update window or Warning window.")
                    warning_message_disconnected = self.device_actions.get_warning_message_disconnected()
                    sleep(2)
                    ...
                    if 'disconnected or in the unmanaged state' not in warning_message_disconnected.text:
                        if perform_update:
                            ...
                            if update_cb:
                                if update_cb.is_selected():
                                    self.utils.print_info(
                                        "Network policy and configuration checkbox is already selected")
                                    sleep(2)
                                else:
                                    self.utils.print_info("Clicking network policy and configuration checkbox")
                                    self.auto_actions.click(update_cb)
                            ...
                            if option.lower() == "enable":
                                self.utils.print_info(
                                    "Proceed yes that user wants to continue with reboot/revert option")
                                self.auto_actions.click(self.devices_web_elements.get_devices_update_yes_btn())
                                self.utils.wait_till(timeout=2)
