...
class NetworkElementHostUtilsKeywords(NetworkElementKeywordBaseClass):
    ...
    def get_ports_from_dut(self, dut):
        try:
            ...
            if dut.cli_type.upper() == "EXOS":
                ...
                output = self.devCmd.send_cmd(dut.name, f'show ports info', max_wait=10, interval=2)[
                    0].return_text
                output = re.findall(r"\r\n(\d+)\s+", output)

            elif dut.cli_type.upper() == "VOSS":
                output = \
                    self.devCmd.send_cmd(dut.name, "show int gig int | no-more", max_wait=10, interval=2)[
                        0].return_text
                output = re.findall(r"\r\n(\d+/\d+)\s+", output)
        finally:
            self.close_connection_with_error_handling(dut)
            ...
            try:
                if dut.cli_type.upper() == "VOSS":
                    for session_id in range(7):
                        self.devCmd.send_cmd(dut.name, f"clear ssh {session_id}")
                elif dut.cli_type.upper() == "EXOS":
                    self.devCmd.send_cmd(dut.name, "clear session all")
            except Exception as err:
                print(err)

            self.network_manager.device_collection.remove_device(dut.name)
    ...
    def set_lldp(self, dut, ports, action="enable"):
        ...
        try:
            ...
            if dut.cli_type.upper() == "EXOS":
                if action == "enable":
                    self.devCmd.send_cmd(dut.name, 'enable cdp ports all', max_wait=10, interval=2)
                    self.devCmd.send_cmd(dut.name, 'enable edp ports all', max_wait=10, interval=2)
                    self.devCmd.send_cmd(dut.name, 'enable lldp ports all', max_wait=10, interval=2)
                elif action == "disable":
                    self.devCmd.send_cmd(dut.name, 'disable cdp ports all', max_wait=10, interval=2)
                    self.devCmd.send_cmd(dut.name, 'disable edp ports all', max_wait=10, interval=2)
                    self.devCmd.send_cmd(dut.name, 'disable lldp ports all', max_wait=10, interval=2)
                ...
                if action == "enable":
                    self.devCmd.send_cmd(dut.name, "no auto-sense enable", max_wait=10, interval=2)
                    self.devCmd.send_cmd(dut.name, "no fa enable", max_wait=10, interval=2)
                    self.devCmd.send_cmd(dut.name, cmd_action, max_wait=10, interval=2)
                    self.devCmd.send_cmd(dut.name, "fa enable", max_wait=10, interval=2)
                    self.devCmd.send_cmd(dut.name, "auto-sense enable", max_wait=10, interval=2)
                elif action == "disable":
                    self.devCmd.send_cmd(dut.name, "no " + cmd_action, max_wait=10, interval=2)
