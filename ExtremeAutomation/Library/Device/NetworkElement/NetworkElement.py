    ...
    def login(self):
        ...
        while not logged_in and retries < self.max_connection_retries:
            ...
            try:
                ...
                if not logged_in and self.oper_sys == 'AH-AP' and \
                        not self.current_agent.get_enable_fallback_password_mode():
                    self.current_agent.enable_fallback_password_mode(True)
                    logged_in = self.current_agent.login()
                    self.current_agent.enable_fallback_password_mode(False)
                if not logged_in:
