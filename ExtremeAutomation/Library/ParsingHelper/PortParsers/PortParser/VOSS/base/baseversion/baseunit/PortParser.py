...
class PortParser(PortParserBaseClass):
    ...
    def strip_and_store_flags(self, port):
        ...
        if "emptyCliReturn" in port or "notpresent" in port.lower():
            ...
            for portrange in portlist:
                ...
                if len(portrangelist) == 3:
                    portslot = portrangelist[0]  # If the port uses the stacked format, populate slot and port
                    portrange = portrangelist[1].split("-")[0] + "-" + portrangelist[2]
                elif len(portrangelist) == 2:
                    portslot = portrangelist[0]
    ...
    @staticmethod
    def strip_voss_flags(port):
        """
        Remove all VOSS specific flags from the port string.
        """
        if "emptyCliReturn" in port or "notpresent" in port.lower():
            return port
        else:
        ...
        for port in portlist.split(","):
            if "-" in port:
                ...
                try:
                    ...
                    self.logger.log_error("invalid voss port range: " + port)
    ...
    def expand_master_port_list(self, masterportlist):
        ...
        for portblock in portblocks:
            # media = ""  # VOSS doesn"t seem to display media types
            ...
            for slot in slots:
                ...
                for port in portlist:
                    portvec.append(slot.replace("SLOT", "") + "/" + port)
        self.port_list = sorted(set(portvec))
