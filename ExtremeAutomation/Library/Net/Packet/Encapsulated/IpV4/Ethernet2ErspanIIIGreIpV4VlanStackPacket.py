...
class Ethernet2ErspanIIIGreIpV4VlanStackPacket(Ethernet2GreIpV4VlanStackPacket, ErspanIIIPacketHeader):
    def __init__(self, num_tags, innerpacket=EthernetPacket()):
        super(Ethernet2ErspanIIIGreIpV4VlanStackPacket, self).__init__(num_tags, innerpacket)
        ErspanIIIPacketHeader.__init__(self)
        self.set_name("Ethernet2ErspanIIIGreIpV4VlanStackPacket")

    def to_packet_string(self):
        return super(Ethernet2ErspanIIIGreIpV4VlanStackPacket, self).to_packet_string() + \
            ErspanIIIPacketHeader.to_packet_string(self)

    def build(self):
        super(Ethernet2ErspanIIIGreIpV4VlanStackPacket, self).build()
    ...
    def get_header_length(self):
        ...
        if "1" == val:
            header += 8
        return super(Ethernet2ErspanIIIGreIpV4VlanStackPacket, self).get_header_length() + header

    def get_hltapi_commands(self):
        cmd_dict = super(Ethernet2ErspanIIIGreIpV4VlanStackPacket, self).get_hltapi_commands()
        cmd_dict.update(ErspanIIIPacketHeader.get_hltapi_commands(self))
        return cmd_dict

    def set_payload(self, payload):
        payload = payload.replace(" ", "")
        super(Ethernet2ErspanIIIGreIpV4VlanStackPacket, self).set_payload(payload)
        ErspanIIIPacketHeader.parse_bytes(self)

    def get_header_bytes(self, break_up_header=False):
        temp_bytes = super(Ethernet2ErspanIIIGreIpV4VlanStackPacket, self).get_header_bytes(break_up_header)
        if break_up_header:
            temp_bytes += "|"
    ...
    def config_thirdparty_traffic_generator_stream(self, tgen_type, generator_ref, port_string, stream_id, **kwargs):
        super(Ethernet2ErspanIIIGreIpV4VlanStackPacket, self).config_thirdparty_traffic_generator_stream(
