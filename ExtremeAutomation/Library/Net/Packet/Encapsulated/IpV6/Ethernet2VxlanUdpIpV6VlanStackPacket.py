...
class Ethernet2VxlanUdpIpV6VlanStackPacket(Ethernet2UdpIpV6VlanStackPacket, VxlanPacketHeader):
    def __init__(self, num_tags, innerpacket=default_packet):
        super(Ethernet2VxlanUdpIpV6VlanStackPacket, self).__init__(num_tags)
        VxlanPacketHeader.__init__(self)
        self.set_vxlan_inner_packet(innerpacket)
        self.set_name("Ethernet2VxlanUdpIpV6VlanStackPacket")

    def to_packet_string(self):
        return super(Ethernet2VxlanUdpIpV6VlanStackPacket, self).to_packet_string() + \
            VxlanPacketHeader.to_packet_string(self)

    def build(self):
        super(Ethernet2VxlanUdpIpV6VlanStackPacket, self).build()
        VxlanPacketHeader.build(self)

    def get_header_length(self):
        return super(Ethernet2VxlanUdpIpV6VlanStackPacket, self).get_header_length() + self.HEADER_SIZE_VXLAN + \
            self.get_vxlan_inner_packet_length()

    def get_hltapi_commands(self):
        cmd_dict = super(Ethernet2VxlanUdpIpV6VlanStackPacket, self).get_hltapi_commands()
        cmd_dict.update(VxlanPacketHeader.get_hltapi_commands(self))
        return cmd_dict

    def set_payload(self, payload):
        payload = payload.replace(" ", "")
        super(Ethernet2VxlanUdpIpV6VlanStackPacket, self).set_payload(payload)
        VxlanPacketHeader.parse_bytes(self)

    def get_header_bytes(self, break_up_header=False):
        temp_bytes = super(Ethernet2VxlanUdpIpV6VlanStackPacket, self).get_header_bytes(break_up_header)
        if break_up_header:
            temp_bytes += "|"
    ...
    def config_thirdparty_traffic_generator_stream(self, tgen_type, generator_ref, port_string, stream_id, **kwargs):
        super(Ethernet2VxlanUdpIpV6VlanStackPacket, self).config_thirdparty_traffic_generator_stream(
