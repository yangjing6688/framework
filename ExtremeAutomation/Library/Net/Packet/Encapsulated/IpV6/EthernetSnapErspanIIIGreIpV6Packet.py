...
class EthernetSnapErspanIIIGreIpV6Packet(EthernetSnapGreIpV6Packet, ErspanIIIPacketHeader):
    def __init__(self, innerpacket=EthernetPacket()):
        super(EthernetSnapErspanIIIGreIpV6Packet, self).__init__(innerpacket)
        ErspanIIIPacketHeader.__init__(self)
        self.set_name("EthernetSnapErspanIIIGreIpV6Packet")

    def to_packet_string(self):
        return super(EthernetSnapErspanIIIGreIpV6Packet, self).to_packet_string() + \
            ErspanIIIPacketHeader.to_packet_string(self)

    def build(self):
        super(EthernetSnapErspanIIIGreIpV6Packet, self).build()
        ErspanIIIPacketHeader.build(self)

    def get_header_length(self):
        return super(EthernetSnapErspanIIIGreIpV6Packet, self).get_header_length() + self.HEADER_SIZE_ERSPANIII

    def get_hltapi_commands(self):
        cmd_dict = super(EthernetSnapErspanIIIGreIpV6Packet, self).get_hltapi_commands()
        cmd_dict.update(ErspanIIIPacketHeader.get_hltapi_commands(self))
        return cmd_dict

    def set_payload(self, payload):
        payload = payload.replace(" ", "")
        super(EthernetSnapErspanIIIGreIpV6Packet, self).set_payload(payload)
        ErspanIIIPacketHeader.parse_bytes(self)

    def get_header_bytes(self, break_up_header=False):
        temp_bytes = super(EthernetSnapErspanIIIGreIpV6Packet, self).get_header_bytes(break_up_header)
        if break_up_header:
            temp_bytes += "|"
    ...
    def config_thirdparty_traffic_generator_stream(self, tgen_type, generator_ref, port_string, stream_id, **kwargs):
        super(EthernetSnapErspanIIIGreIpV6Packet, self).config_thirdparty_traffic_generator_stream(
