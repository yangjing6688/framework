...
class RstpPacketHeader(object):
    packet_name = None
    pkt_bytes = None

    def __init__(self):
        self.add_rstp_header()

    def to_packet_string(self):
        table = TableFormatter()
        return "\n\nRSTP" + table.to_table_string()

    def add_rstp_header(self):
        self.register_packet_tag(PacketTagConstants.TAG_RSTP)

    def build(self):
        self.add_rstp_header()
        self.set_bpdu_version(0x02, True)

    def get_hltapi_commands(self):
        dummy_dict = dict()
        return dummy_dict

    def config_thirdparty_traffic_generator_stream_rstp(self, tgen_type, generator_ref, port_string, stream_id,
                                                        **kwargs):
        if tgen_type == EthernetPacketConstants.TRAFFIC_GENERATION_OSTINATO:
            self.config_ostinato_stream_rstp(generator_ref, port_string, stream_id, **kwargs)
        elif tgen_type == EthernetPacketConstants.TRAFFIC_GENERATION_JETS:
            self.config_jet_stream_rstp(generator_ref, port_string, stream_id, **kwargs)
        else:
            return EthernetPacketConstants.TRAFFIC_GENERATION_NOT_SUPPORTED

    def config_ostinato_stream_rstp(self, drone, port_string, stream_id, **kwargs):
    ...
    def config_jets_stream_rstp(self, jets_dev, port_string, stream_id, **kwargs):
        # this is not supported yet
        # do it with rawdata instead
        pkt_bytes = "0x" + RstpPacketHeader.get_header_bytes(self)
        jets_dev.pdl_add_packet_header(JetsDeviceConstants.RAW_DATA, {"data": pkt_bytes})

    def get_ixtclhal_rstp_api_commands(self, port_string, streamid):
    ...
    def get_spirent_rstp_api_commands(self, port_name_stream):
        dummy_dict = []
    ...
    @staticmethod
    def compare_rstp_packet_header(expected, actual, ignore_list, print_results=True, print_failures=True):
        results = True
        try:
            assert isinstance(expected, RstpPacketHeader)
            assert isinstance(actual, RstpPacketHeader)
        except Exception as e:
            results = False
            if print_results or print_failures:
                PacketObject.logger.log_info(e)
                PacketObject.logger.log_error("Error with RstpPacketHeader")
        return results


class RstpPacketConstants:
    def __init__(self):
        pass
