...
class IsisCSNPPacketHeader(object):
    packet_name = None
    pkt_bytes = None
    """
    ISIS CSNP Packet Header
        # pdu_length = 16bits
        # source_id = 56bits
        # start_lsp_id = 64bits
        # end_lsp_id = 64bits
    ...
    # start SNP start_lsp_id methods
    #  start_lsp_id is a 64 bit HEX_STRING example: FFFFFFFFFFFFFFFF
    def set_snp_start_lsp_id(self, start_lsp_id, ignore_check=False):
        start_lsp_id = self.normalize_value(start_lsp_id, PacketConstants.HEX_STRING)
        self.set_packet_component(PacketConstants.PACKET_HEADER_L4_ISIS_SNP, SNPPacketConstants.SNP_START_LSP_ID,
                                  start_lsp_id)

    def get_snp_start_lsp_id(self):
        return self.normalize_value(self.get_packet_component(
            PacketConstants.PACKET_HEADER_L4_ISIS_SNP, SNPPacketConstants.SNP_START_LSP_ID), PacketConstants.HEX_STRING)

    def get_snp_start_lsp_id_hltapi_cmd(self, dummy_dict):
        start_lsp_id = self.get_snp_start_lsp_id()
        if start_lsp_id and 'Not Set' not in start_lsp_id:
            dummy_dict[TrafficConfigConstants.TEMP_START_LSP_ID_CMD] = start_lsp_id
    # end SNP start_lsp_id methods

    # start SNP end_lsp_id methods
    #  end_lsp_id is a 64 bit HEX_STRING example: FFFFFFFFFFFFFFFF
    def set_snp_end_lsp_id(self, end_lsp_id, ignore_check=False):
        end_lsp_id = self.normalize_value(end_lsp_id, PacketConstants.HEX_STRING)
        self.set_packet_component(PacketConstants.PACKET_HEADER_L4_ISIS_SNP, SNPPacketConstants.SNP_END_LSP_ID,
                                  end_lsp_id)

    def get_snp_end_lsp_id(self):
        return self.normalize_value(self.get_packet_component(
            PacketConstants.PACKET_HEADER_L4_ISIS_SNP, SNPPacketConstants.SNP_END_LSP_ID), PacketConstants.HEX_STRING)

    def get_snp_end_lsp_id_hltapi_cmd(self, dummy_dict):
        end_lsp_id = self.get_snp_end_lsp_id()
        if end_lsp_id and 'Not Set' not in end_lsp_id:
            dummy_dict[TrafficConfigConstants.TEMP_END_LSP_ID_CMD] = end_lsp_id
    # end SNP end_lsp_id methods
    ...
    def to_packet_string(self):
        ...
        table.add_row_value("start_lsp_id", self.get_snp_start_lsp_id())
        table.add_row_value("end_lsp_id", self.get_snp_end_lsp_id())
    ...
    #     if self.is_field_set(self.get_snp_start_lsp_id()):
    #         snpField.version = int(self.get_snp_start_lsp_id())
    #     if self.is_field_set(self.get_snp_end_lsp_id()):
    #         snpField.version = int(self.get_snp_end_lsp_id())

    def config_jets_stream_snp(self, jets_dev, port_string, stream_id, **kwargs):
        pkt_fields = {}
        header_name = "SNP_HDR"
        pkt_bytes = "0x" + IsisCSNPPacketHeader.get_header_bytes(self)
        ...
        if self.is_field_set(self.get_snp_start_lsp_id()):
            pkt_fields["start_lsp_id"] = int(self.get_snp_start_lsp_id())
        if self.is_field_set(self.get_snp_end_lsp_id()):
            pkt_fields["end_lsp_id"] = int(self.get_snp_end_lsp_id())
    ...
    def parse_bytes(self):
        ...
        start_lsp_id = self.get_bits_from_string(64, payload)
        self.set_snp_start_lsp_id("0x" + start_lsp_id)
        payload = self.remove_bits_from_string(64, payload)
        end_lsp_id = self.get_bits_from_string(64, payload)
        self.set_snp_end_lsp_id("0x" + end_lsp_id)
        payload = self.remove_bits_from_string(64, payload)
        # processTLVs
    ...
    def get_header_bytes(self):
        ...
        ret_string += self.format_byte_string(self.get_snp_source_id(), PacketConstants.HEX_STRING, 56)
        ret_string += self.format_byte_string(self.get_snp_start_lsp_id(), PacketConstants.HEX_STRING, 64)
        ret_string += self.format_byte_string(self.get_snp_end_lsp_id(), PacketConstants.HEX_STRING, 64)
    ...
    def compare_snp_packet_header(expected, actual, ignore_list, print_results=True, print_failures=True):
        results = True
        try:
            assert isinstance(expected, IsisCSNPPacketHeader)
            assert isinstance(actual, IsisCSNPPacketHeader)
