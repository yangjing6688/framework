...
class PortCustomShowTools(PortBaseCustomShowTools):
    ...
    def create_interface_dictionaries(self, output, args, **kwargs):
        output = output.replace("SNMPv2-SMI::mib-2.31.1.1.1.1.", "")
        output = output.replace("Slot ", "")
        output = output.replace("VLAN-", "")
        output = output.replace(", Port ", "/")
        ...
        result = len(if_name_to_if_index) and len(if_index_to_if_name) != 0
    ...
    def verify_port_operational(self, output, args, **kwargs):
        """
        The following function returns True if the provided port is operationally up.
        """
    ...
    def verify_port_mtu(self, output, args, **kwargs):
        """
        The following function returns True if the port mtu setting is expected.
        """
    ...
    def verify_port_high_speed(self, output, args, **kwargs):
        """
        The following function returns True if the port high speed setting is expected.
        """
    ...
    def verify_port_mac_address(self, output, args, **kwargs):
        """
        The following function returns True if the port mac address is expected.
        """
    ...
    def create_dot1d_dictionaries(self, output, args, **kwargs):
        if output != "NO DATA":
            # Note: Change next line to dot1dBasePortIfIndex (i.e. 4.1.1 to 4.1.2) once it is fixed.
            # Also change show_dot1d_port_ifindex VOSS in csv.
            output = output.replace("SNMPv2-SMI::mib-2.17.1.4.1.1.", "").replace("=", "")
            output = output.split()
    ...
    def check_port_flex_uni_status(self, output, args, **kwargs):
        ...
        return parse_result == "1", {"ret_flex_uni_status": parse_result}
